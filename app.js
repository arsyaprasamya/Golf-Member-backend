...
app.get("/", (req, res) => {
  var sql = `SELECT customer.status AS status, customer.tempat_lahir AS lahir,
...
app.post("/penghasilanedit/:id", (req, res) => {
  console.log(req.body);
  var data = {
    id: req.body.id,
    status: req.body.status,
...
//ambil status anak by id
app.get("/status_anak/:id", (req, res) => {
  var sql = `SELECT status FROM status_anak WHERE id = ${req.params.id}`;
  db.query(sql, (err, result) => {
    if (err) {
      throw err;
    } else {
